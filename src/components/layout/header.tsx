...
export function Header() {
  ...
  useEffect(() => {
    // In a real app, you would filter for the current user
          ...
          <Sheet>
            ...
            <SheetContent side="left" className="pr-0 bg-black">
              ...
              <nav className="flex flex-col space-y-4 p-6 pt-4">
                {[...navLinks, { href: "/notifications", label: "सूचनाएं" }, { href: "/offers", label: "ऑफर्स" }, { href: "/partner", label: "पार्टनर बनें" }].map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={cn(
